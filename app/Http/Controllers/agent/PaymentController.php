...
return Payment::make($attributes, function ($then) use ($data) {
    ...
    if($maindata['image'] != NULL && $maindata['featured'] != NULL){
       ...
       if(!$valid->fails()){
        // return dd($valid);
        $type = DB::Table('types')->where('name',$maindata['type'])->get();
        ...
        $agentdata = DB::table('agents')->where('id',$maindata['Agent_name'])->get();
        ...
        if($property_id){
            ...
            $then->redirectRoute('agent.payment.status',$datatoforward);
        }
        else{
...
// Get the payment status.
  ...
  # Returns boolean - true / false
    ...
    if($check ==false):
       ...
       ->get();
     // return dd($property);
       $agent = DB::table('agents')->where('id',$property[0]->agent_id)->get();
    ...
    $prop_count = DB::table('agent_property')->where('agent_id',$agent[0]->id)->count();
    $query = DB::table('agents')->where('id',$agent[0]->id)->update(['property_counts'=>$prop_count]);
